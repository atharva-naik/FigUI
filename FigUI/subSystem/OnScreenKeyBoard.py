...
from PyQt5.QtWidgets import QWidget, QPushButton, QMainWindow, QApplication, QLineEdit
...
class KeyButton(QPushButton):
    ...
    def __init__(self, key: str, parent=Union[QWidget, None]):
        ''' 
        KeyButton class constructor
        Parameters
            key : (str) key of the button
            parent : (Union[QWidget, None]) parent widget
        '''
...
class AlphaNeumericVirtualKeyboard(QWidget):
...
class FigVirtualKeyBoard(QMainWindow):
    def __init__(self, source, parent=None):
        super(FigVirtualKeyBoard, self).__init__(parent)
        self.osk = AlphaNeumericVirtualKeyboard(source)
        self.setCentralWidget(self.osk)
...
if __name__ == "__main__":
    ...
    testWindow.show()
    osk = FigVirtualKeyBoard(testWindow.textEdit)
    sys.exit(app.exec_())